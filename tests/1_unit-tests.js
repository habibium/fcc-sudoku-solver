...
suite("UnitTests", () => {
  suite("checkRowPlacement tests", function () {
    test("true tests", function () {
      assert.isTrue(
        solver.checkRowPlacement(testPuzzles[0][0], "D", 4, 8),
        "Upper case row index failed"
      );
      assert.isTrue(
        solver.checkRowPlacement(testPuzzles[0][0], "f", 9, 4),
        "Lower case row index failed"
      );
    });
  ...
  suite("findAllCellOptions tests", function () {
    test("Finds inputs that don't violate sudoku rules", function () {
      assert.deepEqual(solver.findAllCellOptions(testPuzzles[3][0], "A", 1), [
        "3",
        "4",
      ]);
    });
